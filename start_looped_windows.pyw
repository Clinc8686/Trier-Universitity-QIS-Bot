...
global username, password, modulnumbers, filepath, discord_webhook_url, ifttt_url, seconds
filepath = os.getcwd()
...
#REQUIRED! change below the seconds you want to refresh the QIS, must greater than 60 seconds. recommended is 300
seconds = 300
...
def AddToRegistry():    #Setzt den Autostart in der Registry
    pth = str(os.path.dirname(os.path.abspath(__file__))+"\\"+os.path.basename(__file__))
    key = reg.HKEY_CURRENT_USER
    key_value = "Software\Microsoft\Windows\CurrentVersion\Run"
    open = reg.OpenKey(key, key_value, 0, reg.KEY_ALL_ACCESS)
    reg.SetValueEx(open, str(os.path.basename(__file__)), 0, reg.REG_SZ, pth)
    reg.CloseKey(open)
...
if __name__ == "__main__":
    ...
    if seconds <= 60:
        sys.exit('Smallest value of seconds is 15!')
